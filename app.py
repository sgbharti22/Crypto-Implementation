...
#computes base^exp mod modulus
	def modexp( base, exp, modulus ):
		# print("pow : ",pow(base, exp, modulus))
		return pow(base, exp, modulus)

#solovay-strassen primality test.  tests if num is prime
	def SS( num, iConfidence ):		#p g x h == p e1 d e2
...
#computes the jacobi symbol of a, n
	def jacobi( a, n ):		#p g x h == p e1 d e2
		if a == 0:
				if n == 1:
						return 1
				else:
...
#this function was implemented from the algorithm described here:
	...
	def find_primitive_root( p ):	#p g x h == p e1 d e2
		if p == 2:
				return 1
...
#find n bit prime
	def find_prime(iNumBits, iConfidence):
		#keep testing until one is found     #p g x h == p e1 d e2
		while(1):
...
#generates public key K1 (p, g, h) and private key K2 (p, g, x)
	def generate_keys(iNumBits=256, iConfidence=32):
		#p (p) is the primes					(p)
		#g (e1)is the primitve roots			(e1)
		#x  (d)is random in (0, p-1) inclusive  (d)
		#h  (e2) = g ^ x mod p  					(e2)
		...
		print("\nd (bob's private key): ",x)
		print("\ne2 : ",h)	#p g x h == p e1 d e2
...
#p g x h == p e1 d e2
	#cipher_pairs list will hold pairs (c, d) corresponding to each integer in z
		...
		for i in z:
				...
				y = random.randint( 0, key.p )  # y == r
				#c = g^y mod p
				print("\n\ny (alice's private key): ",y)
				c = modexp( key.g, y, key.p )
				#d = ih^y mod p
				d = (i*modexp( key.h, y, key.p)) % key.p   # c,d == c1,c2
...
#p g x h == p e1 d e2
	...
	if request.method=="POST":
		...
		message = request.form['input-alice']
	
		cipher = encrypt(pub, message)
		print("\n\nEncrypted plain text :",cipher)
		plain = decrypt(priv, cipher)
		print("\n\nDecrepted plain text: ",plain)

	return render_template('index.html',ci=cipher,plain=plain)
